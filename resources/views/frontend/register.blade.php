...
@section('content')
    <section class="about-banner">
        @if (!empty($register_banner->banner_image))
            <img src="{{ asset($register_banner->banner_image) }}"
                alt="{{ $register_banner->title ?? 'About Us Background' }}">
        @else
            <img src="{{ asset('frontend/assets/image/japan.jpg') }}" alt="Default Background">
        @endif
        <div class="banner-content">
            <div class="banner-content-inner">
                <h1>{{ $register_banner->title ?? 'About Us' }}</h1>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb justify-content-center">
                        <li class="breadcrumb-item"><a href="{{ route('frontend.home') }}">Home</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{ $register_banner->title ?? 'About Us' }}
                        </li>
                    </ol>
                </nav>
            </div>
    ...
    <div class="container mt-5 mb-5 shadow rounded p-4">
        ...
        <form id="enquiryForm" method="POST" action="{{ route('frontend.register.submit') }}" enctype="multipart/form-data"
            ...
            <div class="my-6">
                <a href="">
                    <button class="custom-btn btn1"><span>Register <i
